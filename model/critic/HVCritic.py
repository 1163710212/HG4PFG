...
class HVCritic(nn.Module):
    ...
    def high_value(self, h_state):
        return self.h_net(h_state).view(-1)

    def low_value(self, l_state, h_action):
        weighted_h_action = nn.functional.relu(h_action).detach()
        return self.l_net(
            torch.cat([l_state, weighted_h_action], dim=1)
        ).view(-1)
    ...
    def forward(self, feed_dict):
        ...
        H_V = self.high_value(h_state_emb)
        L_V = self.low_value(l_state_emb, t_h_action)
