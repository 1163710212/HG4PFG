...
class BackboneUserEncoder(BaseModel):
    ...
    def __init__(self, args, reader_stats, has_up=False):
        ...
        self.state_dim = 3 * self.enc_dim + (1 if has_up else 0)
...
#                                 dropout_rate = args.dropout_rate, do_batch_norm = True)
    ...
    def encode_state(self, feed_dict, B):
        ...
        #
        # PyTorch 1.12's eval/no-grad fast path can misinterpret the existing
        # 2-D causal mask as a batch-wise attention mask and fail with
        # ``Expected attn_mask->sizes()[0] == batch_size``.  An all-false
        # padding mask is semantically neutral (no token is padding here) and
        # keeps TransformerEncoder on the correct, mask-compatible path.  This
        # matters for DQN target networks, which are intentionally evaluated
        # under no_grad.
        ...
        # user state (B, 3*enc_dim + 1) combines user history and user profile features
        #print(f'mush{pop_prefer}')
        #state = torch.cat([hist_enc, user_enc], 1)

        if self.has_up:
            pop_prefer = feed_dict['user_pop_prefer'].reshape(-1, 1)
            state = self.mlp(torch.cat([hist_enc, user_enc, pop_prefer], 1))
        else:
            state = torch.cat([hist_enc, user_enc], 1)
        # (B, enc_dim)
