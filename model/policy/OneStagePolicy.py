...
class OneStagePolicy(BaseModel):
    ...
    def to(self, device):
        ...
        if hasattr(self, 'h_user_encoder'):
            self.h_user_encoder = self.h_user_encoder.to(device)
        if hasattr(self, 'l_user_encoder'):
            self.l_user_encoder = self.l_user_encoder.to(device)
        return new_self
