...
class OnPolicyBuffer(BaseBuffer):
    ...
    def reset(self, *reset_args):
        ...
        self.buffer['policy_output']['action'] = torch.zeros(self.buffer_size, actor.action_dim) \
            .to(torch.float).to(self.device)
        self.buffer['policy_output']['action_log_prob'] = torch.zeros(self.buffer_size, 1) \
            .to(torch.float).to(self.device)
    ...
    def sample(self):
        ...
        """
        # get indices
        indices = torch.arange(self.current_buffer_size, device=self.device)
        # observation
        ...
        policy_output = {"state": self.buffer["policy_output"]["state"][indices],
                         ...
                         "action_log_prob": self.buffer["policy_output"]["action_log_prob"][indices],
                         "hyper_action": self.buffer["policy_output"]["action"][indices]}  # main change to BaseBuffer
        # user response
    ...
    def update(self, observation, policy_output, user_feedback, next_observation):
        ...
        self.buffer['policy_output']['action_log_prob'][indices] = policy_output['action_log_prob'].view(B, -1)
