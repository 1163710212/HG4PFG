...
class BaseBuffer():
    ...
    def reset(self, *reset_args):
        ...
        policy_output = {'state': torch.zeros(self.buffer_size, actor.state_dim).to(torch.float).to(self.device),
                         'h_state': torch.zeros(self.buffer_size, actor.state_dim).to(torch.float).to(self.device),
                         'l_state': torch.zeros(self.buffer_size, actor.state_dim).to(torch.float).to(self.device),
                         'action': torch.zeros(self.buffer_size, actor.action_dim).to(torch.long).to(self.device),
                         'prob': torch.zeros(self.buffer_size, env.slate_size).to(torch.float).to(self.device)}
