...
class BaseModel(nn.Module):
    ...
    def load_from_checkpoint(self, model_path='', with_optimizer=True):
        ...
        # Inference/final evaluation does not need the optimizer state on the
        # GPU.  Deserialize the checkpoint on CPU in that case, then let
        # load_state_dict copy only model parameters to the model's device.
        # This avoids a transient second copy of the model and Adam state on
        # an already busy training GPU.
        map_location = self.device if with_optimizer else "cpu"
        checkpoint = torch.load(model_path + ".checkpoint", map_location=map_location)
