...
def load_candidate_aligned_item_metadata(dataset_dir, reader):
    """Align row-only popularity metadata with the reader's item vocabulary.

    The auxiliary CSVs are emitted in sorted raw-item-ID order, while the
    response-model vocabulary follows first appearance in the interaction log.
    Validate the proposed reordering against popularity reconstructed from the
    same log before returning any labels.
    """
    popularity = pd.read_csv(
        os.path.join(dataset_dir, 'item_popularity.csv')
    )['popularity'].to_numpy(dtype=np.float64)
    item_types = pd.read_csv(
        os.path.join(dataset_dir, 'item_types.csv')
    )['item_type'].to_numpy(dtype=np.int64)
    raw_item_ids = np.asarray(reader.items)
    if len(popularity) != len(raw_item_ids) or len(item_types) != len(raw_item_ids):
        raise ValueError('item popularity metadata does not match the item vocabulary')
    if len(np.unique(raw_item_ids)) != len(raw_item_ids):
        raise ValueError('reader item vocabulary contains duplicate raw item IDs')

    item_column = next(
        (name for name in ('video_id', 'movie_id') if name in reader.log_data),
        None,
    )
    if item_column is None:
        raise ValueError('cannot identify the raw item-ID column in the reader log')
    counts = reader.log_data[item_column].value_counts(sort=False).reindex(
        raw_item_ids
    ).to_numpy(dtype=np.float64)
    count_range = counts.max() - counts.min()
    if not np.isfinite(counts).all() or count_range <= 0.0:
        raise ValueError('cannot reconstruct non-degenerate item popularity')
    reconstructed = (counts - counts.min()) / count_range

    sorted_raw_ids = np.sort(raw_item_ids)
    auxiliary_row = pd.Series(
        np.arange(len(sorted_raw_ids), dtype=np.int64), index=sorted_raw_ids
    )
    sorted_order = auxiliary_row.loc[raw_item_ids].to_numpy(dtype=np.int64)
    candidates = {
        'already_candidate_order': np.arange(len(raw_item_ids), dtype=np.int64),
        'sorted_raw_item_id': sorted_order,
    }
    errors = {
        name: float(np.max(np.abs(popularity[order] - reconstructed)))
        for name, order in candidates.items()
    }
    alignment_mode = min(errors, key=errors.get)
    if errors[alignment_mode] > 1e-8:
        raise ValueError(
            'item_popularity.csv cannot be aligned to the reader vocabulary; '
            'best max error={0:.3e}'.format(errors[alignment_mode])
        )
    order = candidates[alignment_mode]
    aligned_types = item_types[order]
    if not set(np.unique(aligned_types)).issubset({0, 1}):
        raise ValueError('item_types.csv must be binary (1=popular, 0=long-tail)')
    aligned_popularity = popularity[order]
    popularity_percentile = pd.Series(aligned_popularity).rank(
        method='average', pct=True
    ).to_numpy(dtype=np.float64)
    return {
        'item_types': aligned_types,
        'item_popularity': aligned_popularity,
        'popularity_percentile': popularity_percentile,
        'alignment_mode': alignment_mode,
        'max_popularity_error': errors[alignment_mode],
        'naive_type_agreement': float(np.mean(item_types == aligned_types)),
    }
...
def padding_and_clip(sequence, max_len, padding_direction='left'):
    if len(sequence) < max_len:
        sequence = [0] * (max_len - len(sequence)) + sequence if padding_direction == 'left' else sequence + [0] * (
