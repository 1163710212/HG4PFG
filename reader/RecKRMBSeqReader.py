...
class RecKRMBSeqReader(BaseReader):
    """
    KuaiRec Multi-Behavior Data Reader
    """
    ...
    def parse_data_args(parser):
        ...
        parser.add_argument('--dataset_dir', type=str, default='',
                            help='directory containing item_popularity.csv, item_types.csv, '
                                 'and user_pop_ratio.csv; '
    ...
    def __init__(self, args):
        ...
        - from BaseReader:
            ...
            - n_worker
        """
        print("initiate KuaiRecMultiBehaior sequence reader")
    ...
    def _read_data(self, args):
        ...
        # Build vocabularies from the full log so both simulator checkpoints use
        # identical user/item ID spaces. Targets and histories are partitioned.
        self.users = list(self.log_data['user_id'].unique())
        self.items = list(self.log_data['video_id'].unique())
        grouped_rows = self.log_data.groupby('user_id', sort=False).groups
        full_user_history = {
            uid: np.asarray(grouped_rows[uid], dtype=np.int64) for uid in self.users
        }
        ...
        self.selected_item_features = ['video_type', 'visible_status', 'upload_type', 'video_tag_name']
        ...
        self.item_vocab = get_onehot_vocab(item_meta_file, self.selected_item_features)
    ...
    def _split_environment_data(self, full_user_history):
        """Create exact, deterministic and temporally disjoint simulator halves."""
        ...
        if rows_to_allocate > 0:
            ...
            train_counts[allocation_order[:rows_to_allocate]] += 1
        ...
        for user_pos, uid in enumerate(self.users):
            ...
            selected = rows[:boundary] if self.environment_split == 'train' else rows[boundary:]
    ...
    def get_response_weights(self):
        ...
        for f in self.response_list:
            counts = self.log_data.loc[self.environment_indices, f].value_counts()
    ...
    def __getitem__(self, idx):
        ...
        user_rows = self.user_history[user_id]
        history_end = np.searchsorted(user_rows, row_id, side='left')
        H_rowIDs = user_rows[max(0, history_end - self.max_hist_seq_len):history_end]
    ...
    def get_user_history(self, H_rowIDs):
        ...
        if L == 0:
            ...
            # Compute the user's current popularity preference for agent training.
            H = H[H['is_click'] > 0]
            item_ids = [self.item_id_vocab[iid] for iid in H['video_id']]
    ...
    def get_item_catalog(self):
        """Return every KuaiRec item once in encoded-ID order."""
